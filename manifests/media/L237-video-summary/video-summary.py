...
class VideoSummaryCore:
    ...
    def __init__(self, data_file: Optional[Path] = None):
        self.data_file = data_file or (DATA_DIR / "video-summary.json")
...
def add(ctx, path, format, size, tags):
    """新增媒體"""
    core = VideoSummaryCore(ctx.obj.get("data_file"))
    result = core.add(path, format=format, size=size, tags=list(tags))
    console.print(Panel(f"[green]✓[/green] {result['message']}", title="影片摘要器"))
...
@cli.command()
@click.pass_context
@click.option("--output", "-o", default="video-summary-export.json")
