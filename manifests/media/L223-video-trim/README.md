...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/video-trim/config.json` |
...
[project.scripts]
video-trim = "video_trim.cli:main"
```
...
```bash
video-trim [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
video-trim run --output json

# 詳細模式
video-trim run --verbose

# 靜音（適合 pipe 到其他工具）
video-trim run --quiet | jq .

# 查看設定
video-trim config show

# 匯出 JSON
video-trim export --output json > result.json
```
...
from .core import VideoTrimCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "video-trim" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "video-trim" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(video-trim run --quiet --output json)
echo $result | jq .data
```
...
def call_video_trim(action="run") -> dict:
    proc = subprocess.run(
        ["video-trim", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
video-trim run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=video_trim --cov-report=term-missing
...
```python
import pytest
from video_trim.core import VideoTrimCore
...
*工具編號 #223 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
