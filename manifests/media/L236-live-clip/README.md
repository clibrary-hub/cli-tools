...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/live-clip/config.json` |
...
```bash
live-clip [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
live-clip run --output json

# 詳細模式
live-clip run --verbose

# 靜音（適合 pipe 到其他工具）
live-clip run --quiet | jq .

# 查看設定
live-clip config show

# 匯出 JSON
live-clip export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "live-clip" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "live-clip" / "data.db"
...
{
  "name": "live-clip",
  "description": "直播切片器。解決問題：直播切片成短影音",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(live-clip run --quiet --output json)
echo $result | jq .data
```
...
def call_live_clip(action="run") -> dict:
    proc = subprocess.run(
        ["live-clip", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
live-clip run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=live_clip --cov-report=term-missing
...
```python
import pytest
from live_clip.core import LiveClipCore
...
*工具編號 #236 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
