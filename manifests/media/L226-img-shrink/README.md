...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/img-shrink/config.json` |
...
[project.scripts]
img-shrink = "img_shrink.cli:main"
```
...
```bash
img-shrink [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
img-shrink run --output json

# 詳細模式
img-shrink run --verbose

# 靜音（適合 pipe 到其他工具）
img-shrink run --quiet | jq .

# 查看設定
img-shrink config show

# 匯出 JSON
img-shrink export --output json > result.json
```
...
from .core import ImgShrinkCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "img-shrink" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "img-shrink" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(img-shrink run --quiet --output json)
echo $result | jq .data
```
...
def call_img_shrink(action="run") -> dict:
    proc = subprocess.run(
        ["img-shrink", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
img-shrink run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=img_shrink --cov-report=term-missing
...
```python
import pytest
from img_shrink.core import ImgShrinkCore
...
*工具編號 #226 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
