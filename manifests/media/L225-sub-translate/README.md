...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/sub-translate/config.json` |
...
[project.scripts]
sub-translate = "sub_translate.cli:main"
```
...
```bash
sub-translate [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
sub-translate run --output json

# 詳細模式
sub-translate run --verbose

# 靜音（適合 pipe 到其他工具）
sub-translate run --quiet | jq .

# 查看設定
sub-translate config show

# 匯出 JSON
sub-translate export --output json > result.json
```
...
from .core import SubTranslateCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "sub-translate" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "sub-translate" / "data.db"
...
class SubTranslateCore:
    """
    核心邏輯類別。
    問題：字幕翻譯
    解法：透過 anthropic, click 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(sub-translate run --quiet --output json)
echo $result | jq .data
```
...
def call_sub_translate(action="run") -> dict:
    proc = subprocess.run(
        ["sub-translate", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
sub-translate run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=sub_translate --cov-report=term-missing

# 快速冒煙測試
sub-translate run --help
sub-translate config show
```
...
```python
import pytest
from sub_translate.core import SubTranslateCore
...
*工具編號 #225 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
