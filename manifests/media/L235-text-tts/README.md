...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/text-tts/config.json` |
...
pip install edge-tts click rich
...
dependencies = ["edge-tts", "click", "rich"]

[project.scripts]
text-tts = "text_tts.cli:main"
```
...
# 指定輸出格式
text-tts run --output json

# 詳細模式
text-tts run --verbose

# 靜音（適合 pipe 到其他工具）
text-tts run --quiet | jq .

# 查看設定
text-tts config show

# 匯出 JSON
text-tts export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "text-tts" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "text-tts" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(text-tts run --quiet --output json)
echo $result | jq .data
```
...
def call_text_tts(action="run") -> dict:
    proc = subprocess.run(
        ["text-tts", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
text-tts run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=text_tts --cov-report=term-missing
...
```python
import pytest
from text_tts.core import TextTtsCore
...
*工具編號 #235 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
