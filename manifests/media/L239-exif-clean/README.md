...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/exif-clean/config.json` |
...
dependencies = ["Pillow", "piexif", "click"]

[project.scripts]
exif-clean = "exif_clean.cli:main"
```
...
```bash
exif-clean [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
exif-clean run --output json

# 詳細模式
exif-clean run --verbose

# 靜音（適合 pipe 到其他工具）
exif-clean run --quiet | jq .

# 查看設定
exif-clean config show

# 匯出 JSON
exif-clean export --output json > result.json
```
...
from .core import ExifCleanCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "exif-clean" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "exif-clean" / "data.db"
...
{
  "name": "exif-clean",
  "description": "EXIF 清除器。解決問題：EXIF 隱私清除",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(exif-clean run --quiet --output json)
echo $result | jq .data
```
...
def call_exif_clean(action="run") -> dict:
    proc = subprocess.run(
        ["exif-clean", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
exif-clean run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=exif_clean --cov-report=term-missing
...
```python
import pytest
from exif_clean.core import ExifCleanCore
...
```json
{
  "name": "exif-clean",
  "version": "0.1.0",
  "description": "EXIF 清除器",
  "category": "L",
  "category_name": "媒體處理",
  "number": 239,
  "pain_point": "EXIF 隱私清除",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "exif-clean",
  "dependencies": ["Pillow", "piexif", "click"],
...
*工具編號 #239 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
