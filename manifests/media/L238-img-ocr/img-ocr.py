...
class ImgOcrCore:
    ...
    def __init__(self, data_file: Optional[Path] = None):
        self.data_file = data_file or (DATA_DIR / "img-ocr.json")
...
def add(ctx, path, format, size, tags):
    """新增媒體"""
    core = ImgOcrCore(ctx.obj.get("data_file"))
    result = core.add(path, format=format, size=size, tags=list(tags))
    console.print(Panel(f"[green]✓[/green] {result['message']}", title="圖片 OCR 器"))
...
@cli.command()
@click.pass_context
@click.option("--output", "-o", default="img-ocr-export.json")
