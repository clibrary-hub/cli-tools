...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/img-ocr/config.json` |
...
pip install pytesseract Pillow click
...
dependencies = ["pytesseract", "Pillow", "click"]

[project.scripts]
img-ocr = "img_ocr.cli:main"
```
...
# 指定輸出格式
img-ocr run --output json

# 詳細模式
img-ocr run --verbose

# 靜音（適合 pipe 到其他工具）
img-ocr run --quiet | jq .

# 查看設定
img-ocr config show

# 匯出 JSON
img-ocr export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "img-ocr" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "img-ocr" / "data.db"
...
class ImgOcrCore:
    ...
    核心邏輯類別。
    問題：圖片 OCR 多語言
    解法：透過 pytesseract, Pillow 實現自動化處理。
    """
...
{
  "name": "img-ocr",
  "description": "圖片 OCR 器。解決問題：圖片 OCR 多語言",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(img-ocr run --quiet --output json)
echo $result | jq .data
```
...
def call_img_ocr(action="run") -> dict:
    proc = subprocess.run(
        ["img-ocr", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=img_ocr --cov-report=term-missing
...
```python
import pytest
from img_ocr.core import ImgOcrCore
...
```json
{
  "name": "img-ocr",
  "version": "0.1.0",
  "description": "圖片 OCR 器",
  "category": "L",
  "category_name": "媒體處理",
  "number": 238,
  "pain_point": "圖片 OCR 多語言",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "img-ocr",
  "dependencies": ["pytesseract", "Pillow", "click"],
...
*工具編號 #238 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
