...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/audio-stt/config.json` |
...
pip install whisper click rich
...
dependencies = ["whisper", "click", "rich"]

[project.scripts]
audio-stt = "audio_stt.cli:main"
```
...
```bash
audio-stt [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
audio-stt run --output json

# 詳細模式
audio-stt run --verbose

# 靜音（適合 pipe 到其他工具）
audio-stt run --quiet | jq .

# 查看設定
audio-stt config show

# 匯出 JSON
audio-stt export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "audio-stt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "audio-stt" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(audio-stt run --quiet --output json)
echo $result | jq .data
```
...
def call_audio_stt(action="run") -> dict:
    proc = subprocess.run(
        ["audio-stt", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
audio-stt run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=audio_stt --cov-report=term-missing
...
```python
import pytest
from audio_stt.core import AudioSttCore
...
*工具編號 #234 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
