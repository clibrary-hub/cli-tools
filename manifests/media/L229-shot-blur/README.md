...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/shot-blur/config.json` |
...
```bash
shot-blur [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
shot-blur run --output json

# 詳細模式
shot-blur run --verbose

# 靜音（適合 pipe 到其他工具）
shot-blur run --quiet | jq .

# 查看設定
shot-blur config show

# 匯出 JSON
shot-blur export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "shot-blur" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "shot-blur" / "data.db"
...
{
  "name": "shot-blur",
  "description": "敏感區塊模糊。解決問題：截圖批次模糊敏感資訊",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(shot-blur run --quiet --output json)
echo $result | jq .data
```
...
def call_shot_blur(action="run") -> dict:
    proc = subprocess.run(
        ["shot-blur", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
shot-blur run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=shot_blur --cov-report=term-missing
...
```python
import pytest
from shot_blur.core import ShotBlurCore
...
```json
{
  ...
  "category_name": "媒體處理",
  "number": 229,
  "pain_point": "截圖批次模糊敏感資訊",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "shot-blur",
  "dependencies": ["Pillow", "click", "rich"],
...
*工具編號 #229 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
