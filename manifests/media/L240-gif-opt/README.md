...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/gif-opt/config.json` |
...
# 指定輸出格式
gif-opt run --output json

# 詳細模式
gif-opt run --verbose

# 靜音（適合 pipe 到其他工具）
gif-opt run --quiet | jq .

# 查看設定
gif-opt config show

# 匯出 JSON
gif-opt export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "gif-opt" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "gif-opt" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(gif-opt run --quiet --output json)
echo $result | jq .data
```
...
def call_gif_opt(action="run") -> dict:
    proc = subprocess.run(
        ["gif-opt", action, "--output", "json", "--quiet"],
...
# 執行測試
pytest tests/ -v --cov=gif_opt --cov-report=term-missing
...
```python
import pytest
from gif_opt.core import GifOptCore
...
*工具編號 #240 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
