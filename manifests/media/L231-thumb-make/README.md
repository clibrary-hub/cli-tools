...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/thumb-make/config.json` |
...
[project.scripts]
thumb-make = "thumb_make.cli:main"
```
...
```bash
thumb-make [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
thumb-make run --output json

# 詳細模式
thumb-make run --verbose

# 靜音（適合 pipe 到其他工具）
thumb-make run --quiet | jq .

# 查看設定
thumb-make config show

# 匯出 JSON
thumb-make export --output json > result.json
```
...
from .core import ThumbMakeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "thumb-make" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "thumb-make" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(thumb-make run --quiet --output json)
echo $result | jq .data
```
...
def call_thumb_make(action="run") -> dict:
    proc = subprocess.run(
        ["thumb-make", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
thumb-make run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=thumb_make --cov-report=term-missing
...
```python
import pytest
from thumb_make.core import ThumbMakeCore
...
*工具編號 #231 ／ 類別 L — 媒體處理 ／ 計畫版本 v1.0 ／ 2026-04-27*
