...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/hvac-rule/config.json` |
...
```bash
hvac-rule [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
hvac-rule run --output json

# 詳細模式
hvac-rule run --verbose

# 靜音（適合 pipe 到其他工具）
hvac-rule run --quiet | jq .

# 查看設定
hvac-rule config show

# 匯出 JSON
hvac-rule export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "hvac-rule" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "hvac-rule" / "data.db"
...
{
  "name": "hvac-rule",
  "description": "空調規則器。解決問題：溫度與冷氣聯動",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(hvac-rule run --quiet --output json)
echo $result | jq .data
```
...
def call_hvac_rule(action="run") -> dict:
    proc = subprocess.run(
        ["hvac-rule", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
hvac-rule run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=hvac_rule --cov-report=term-missing
...
```python
import pytest
from hvac_rule.core import HvacRuleCore
...
*工具編號 #424 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
