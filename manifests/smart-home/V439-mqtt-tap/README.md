...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/mqtt-tap/config.json` |
...
# 指定輸出格式
mqtt-tap run --output json

# 詳細模式
mqtt-tap run --verbose

# 靜音（適合 pipe 到其他工具）
mqtt-tap run --quiet | jq .

# 查看設定
mqtt-tap config show

# 匯出 JSON
mqtt-tap export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "mqtt-tap" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "mqtt-tap" / "data.db"
...
{
  "name": "mqtt-tap",
  "description": "MQTT 監聽器。解決問題：IoT MQTT 訊息檢視",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(mqtt-tap run --quiet --output json)
echo $result | jq .data
```
...
def call_mqtt_tap(action="run") -> dict:
    proc = subprocess.run(
        ["mqtt-tap", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
mqtt-tap run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=mqtt_tap --cov-report=term-missing
...
```python
import pytest
from mqtt_tap.core import MqttTapCore
...
```json
{
  "name": "mqtt-tap",
  "version": "0.1.0",
  "description": "MQTT 監聽器",
  "category": "V",
  "category_name": "智慧家庭與IoT",
  "number": 439,
  "pain_point": "IoT MQTT 訊息檢視",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "mqtt-tap",
  "dependencies": ["paho-mqtt", "click", "rich"],
...
*工具編號 #439 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
