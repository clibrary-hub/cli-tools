...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cam-events/config.json` |
...
[project.scripts]
cam-events = "cam_events.cli:main"
```
...
```bash
cam-events [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cam-events run --output json

# 詳細模式
cam-events run --verbose

# 靜音（適合 pipe 到其他工具）
cam-events run --quiet | jq .

# 查看設定
cam-events config show

# 匯出 JSON
cam-events export --output json > result.json
```
...
from .core import CamEventsCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cam-events" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cam-events" / "data.db"
...
{
  "name": "cam-events",
  "description": "安防事件摘要。解決問題：安防攝影機事件摘要",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cam-events run --quiet --output json)
echo $result | jq .data
```
...
def call_cam_events(action="run") -> dict:
    proc = subprocess.run(
        ["cam-events", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cam-events run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cam_events --cov-report=term-missing
...
```python
import pytest
from cam_events.core import CamEventsCore
...
```json
{
  ...
  "number": 425,
  "pain_point": "安防攝影機事件摘要",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "cam-events",
  "dependencies": ["anthropic", "click", "rich"],
...
*工具編號 #425 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
