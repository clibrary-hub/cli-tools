...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/pet-feed/config.json` |
...
# 指定輸出格式
pet-feed run --output json

# 詳細模式
pet-feed run --verbose

# 靜音（適合 pipe 到其他工具）
pet-feed run --quiet | jq .

# 查看設定
pet-feed config show

# 匯出 JSON
pet-feed export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "pet-feed" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "pet-feed" / "data.db"
...
{
  "name": "pet-feed",
  "description": "餵食排程器。解決問題：寵物餵食器排程",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(pet-feed run --quiet --output json)
echo $result | jq .data
```
...
def call_pet_feed(action="run") -> dict:
    proc = subprocess.run(
        ["pet-feed", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
pet-feed run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=pet_feed --cov-report=term-missing
...
```python
import pytest
from pet_feed.core import PetFeedCore
...
*工具編號 #430 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
