...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/lock-log/config.json` |
...
pip install sqlite3 click rich
...
dependencies = ["sqlite3", "click", "rich"]

[project.scripts]
lock-log = "lock_log.cli:main"
```
...
# 指定輸出格式
lock-log run --output json

# 詳細模式
lock-log run --verbose

# 靜音（適合 pipe 到其他工具）
lock-log run --quiet | jq .

# 查看設定
lock-log config show

# 匯出 JSON
lock-log export --output json > result.json
```
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "lock-log" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "lock-log" / "data.db"
...
{
  "name": "lock-log",
  "description": "門鎖紀錄器。解決問題：門鎖紀錄與通知",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(lock-log run --quiet --output json)
echo $result | jq .data
```
...
def call_lock_log(action="run") -> dict:
    proc = subprocess.run(
        ["lock-log", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
lock-log run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=lock_log --cov-report=term-missing
...
```python
import pytest
from lock_log.core import LockLogCore
...
*工具編號 #426 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
