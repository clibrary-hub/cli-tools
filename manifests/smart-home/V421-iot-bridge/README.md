...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/iot-bridge/config.json` |
...
[project.scripts]
iot-bridge = "iot_bridge.cli:main"
```
...
```bash
iot-bridge [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
iot-bridge run --output json

# 詳細模式
iot-bridge run --verbose

# 靜音（適合 pipe 到其他工具）
iot-bridge run --quiet | jq .

# 查看設定
iot-bridge config show

# 匯出 JSON
iot-bridge export --output json > result.json
```
...
from .core import IotBridgeCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "iot-bridge" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "iot-bridge" / "data.db"
...
{
  "name": "iot-bridge",
  "description": "IoT 橋接器。解決問題：多生態系裝置整合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(iot-bridge run --quiet --output json)
echo $result | jq .data
```
...
def call_iot_bridge(action="run") -> dict:
    proc = subprocess.run(
        ["iot-bridge", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
iot-bridge run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=iot_bridge --cov-report=term-missing
...
```python
import pytest
from iot_bridge.core import IotBridgeCore
...
*工具編號 #421 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
