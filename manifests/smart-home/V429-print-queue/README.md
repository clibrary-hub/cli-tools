...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/print-queue/config.json` |
...
pip install click rich subprocess
...
[project.scripts]
print-queue = "print_queue.cli:main"
```
...
```bash
print-queue [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
print-queue run --output json

# 詳細模式
print-queue run --verbose

# 靜音（適合 pipe 到其他工具）
print-queue run --quiet | jq .

# 查看設定
print-queue config show

# 匯出 JSON
print-queue export --output json > result.json
```
...
from .core import PrintQueueCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "print-queue" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "print-queue" / "data.db"
...
{
  "name": "print-queue",
  "description": "列印佇列器。解決問題：印表機任務管理",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(print-queue run --quiet --output json)
echo $result | jq .data
```
...
def call_print_queue(action="run") -> dict:
    proc = subprocess.run(
        ["print-queue", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
print-queue run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=print_queue --cov-report=term-missing

# 快速冒煙測試
print-queue run --help
print-queue config show
```
...
```python
import pytest
from print_queue.core import PrintQueueCore
...
*工具編號 #429 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
