...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/home-backup/config.json` |
...
pip install shutil schedule click
...
dependencies = ["shutil", "schedule", "click"]

[project.scripts]
home-backup = "home_backup.cli:main"
```
...
```bash
home-backup [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
home-backup run --output json

# 詳細模式
home-backup run --verbose

# 靜音（適合 pipe 到其他工具）
home-backup run --quiet | jq .

# 查看設定
home-backup config show

# 匯出 JSON
home-backup export --output json > result.json
```
...
from .core import HomeBackupCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "home-backup" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "home-backup" / "data.db"
...
class HomeBackupCore:
    """
    核心邏輯類別。
    問題：家庭備份排程
    解法：透過 shutil, schedule 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(home-backup run --quiet --output json)
echo $result | jq .data
```
...
def call_home_backup(action="run") -> dict:
    proc = subprocess.run(
        ["home-backup", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
home-backup run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=home_backup --cov-report=term-missing

# 快速冒煙測試
home-backup run --help
home-backup config show
```
...
```python
import pytest
from home_backup.core import HomeBackupCore
...
*工具編號 #436 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
