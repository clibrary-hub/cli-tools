...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/home-traffic/config.json` |
...
dependencies = ["scapy", "click", "rich"]

[project.scripts]
home-traffic = "home_traffic.cli:main"
```
...
```bash
home-traffic [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
home-traffic run --output json

# 詳細模式
home-traffic run --verbose

# 靜音（適合 pipe 到其他工具）
home-traffic run --quiet | jq .

# 查看設定
home-traffic config show

# 匯出 JSON
home-traffic export --output json > result.json
```
...
from .core import HomeTrafficCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "home-traffic" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "home-traffic" / "data.db"
...
class HomeTrafficCore:
    """
    核心邏輯類別。
    問題：路由器流量分析
    解法：透過 scapy, click 實現自動化處理。
    """
...
{
  "name": "home-traffic",
  "description": "家用流量分析。解決問題：路由器流量分析",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(home-traffic run --quiet --output json)
echo $result | jq .data
```
...
def call_home_traffic(action="run") -> dict:
    proc = subprocess.run(
        ["home-traffic", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
home-traffic run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=home_traffic --cov-report=term-missing

# 快速冒煙測試
home-traffic run --help
home-traffic config show
```
...
```python
import pytest
from home_traffic.core import HomeTrafficCore
...
*工具編號 #435 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
