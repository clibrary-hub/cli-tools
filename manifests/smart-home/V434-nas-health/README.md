...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/nas-health/config.json` |
...
[project.scripts]
nas-health = "nas_health.cli:main"
```
...
```bash
nas-health [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
nas-health run --output json

# 詳細模式
nas-health run --verbose

# 靜音（適合 pipe 到其他工具）
nas-health run --quiet | jq .

# 查看設定
nas-health config show

# 匯出 JSON
nas-health export --output json > result.json
```
...
from .core import NasHealthCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "nas-health" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "nas-health" / "data.db"
...
{
  "name": "nas-health",
  "description": "NAS 健康檢查。解決問題：NAS 健康檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(nas-health run --quiet --output json)
echo $result | jq .data
```
...
def call_nas_health(action="run") -> dict:
    proc = subprocess.run(
        ["nas-health", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
nas-health run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=nas_health --cov-report=term-missing
...
```python
import pytest
from nas_health.core import NasHealthCore
...
*工具編號 #434 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
