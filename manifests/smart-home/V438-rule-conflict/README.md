...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/rule-conflict/config.json` |
...
[project.scripts]
rule-conflict = "rule_conflict.cli:main"
```
...
```bash
rule-conflict [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
rule-conflict run --output json

# 詳細模式
rule-conflict run --verbose

# 靜音（適合 pipe 到其他工具）
rule-conflict run --quiet | jq .

# 查看設定
rule-conflict config show

# 匯出 JSON
rule-conflict export --output json > result.json
```
...
### `rule_conflict/cli.py`

```python
import click
from rich.console import Console
from .core import RuleConflictCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "rule-conflict" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "rule-conflict" / "data.db"
...
{
  "name": "rule-conflict",
  "description": "規則衝突器。解決問題：自動化規則衝突檢查",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(rule-conflict run --quiet --output json)
echo $result | jq .data
```
...
def call_rule_conflict(action="run") -> dict:
    proc = subprocess.run(
        ["rule-conflict", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
rule-conflict run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=rule_conflict --cov-report=term-missing

# 快速冒煙測試
rule-conflict run --help
rule-conflict config show
```
...
```python
import pytest
from rule_conflict.core import RuleConflictCore
...
```json
{
  ...
  "number": 438,
  "pain_point": "自動化規則衝突檢查",
  "commercial": "✅",
  "language": "Python",
  "entry_point": "rule-conflict",
  "dependencies": ["click", "rich", "pydantic"],
...
*工具編號 #438 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
