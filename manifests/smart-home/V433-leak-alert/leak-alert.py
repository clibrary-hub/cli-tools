...
class LeakAlertCore:
    ...
    def __init__(self, config_file: Optional[Path] = None):
        self.config_file = config_file or (CONFIG_DIR / "leak-alert.json")
...
def register(ctx, device_id, name, enabled, schedule):
    """註冊裝置"""
    core = LeakAlertCore(ctx.obj.get("config"))
    result = core.register(device_id, name, enabled=enabled, schedule=schedule)
    console.print(Panel(f"[green]✓[/green] {result['message']}", title="漏水告警器"))
