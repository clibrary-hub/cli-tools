...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/leak-alert/config.json` |
...
[project.scripts]
leak-alert = "leak_alert.cli:main"
```
...
```bash
leak-alert [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
leak-alert run --output json

# 詳細模式
leak-alert run --verbose

# 靜音（適合 pipe 到其他工具）
leak-alert run --quiet | jq .

# 查看設定
leak-alert config show

# 匯出 JSON
leak-alert export --output json > result.json
```
...
from .core import LeakAlertCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "leak-alert" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "leak-alert" / "data.db"
...
{
  "name": "leak-alert",
  "description": "漏水告警器。解決問題：漏水/瓦斯偵測整合",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(leak-alert run --quiet --output json)
echo $result | jq .data
```
...
def call_leak_alert(action="run") -> dict:
    proc = subprocess.run(
        ["leak-alert", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
leak-alert run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=leak_alert --cov-report=term-missing
...
```python
import pytest
from leak_alert.core import LeakAlertCore
...
```json
{
  ...
  "number": 433,
  "pain_point": "漏水/瓦斯偵測整合",
  "commercial": "⚠️",
  "language": "Python",
  "entry_point": "leak-alert",
  "dependencies": ["paho-mqtt", "click", "rich"],
...
*工具編號 #433 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
