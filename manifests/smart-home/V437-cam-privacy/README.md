...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/cam-privacy/config.json` |
...
[project.scripts]
cam-privacy = "cam_privacy.cli:main"
```
...
```bash
cam-privacy [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
cam-privacy run --output json

# 詳細模式
cam-privacy run --verbose

# 靜音（適合 pipe 到其他工具）
cam-privacy run --quiet | jq .

# 查看設定
cam-privacy config show

# 匯出 JSON
cam-privacy export --output json > result.json
```
...
from .core import CamPrivacyCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "cam-privacy" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "cam-privacy" / "data.db"
...
class CamPrivacyCore:
    """
    核心邏輯類別。
    問題：智慧鏡頭隱私模式
    解法：透過 requests, click 實現自動化處理。
    """
...
{
  "name": "cam-privacy",
  "description": "隱私模式器。解決問題：智慧鏡頭隱私模式",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(cam-privacy run --quiet --output json)
echo $result | jq .data
```
...
def call_cam_privacy(action="run") -> dict:
    proc = subprocess.run(
        ["cam-privacy", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
cam-privacy run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=cam_privacy --cov-report=term-missing

# 快速冒煙測試
cam-privacy run --help
cam-privacy config show
```
...
```python
import pytest
from cam_privacy.core import CamPrivacyCore
...
*工具編號 #437 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
