...
class BlindScheduleCore:
    ...
    def __init__(self, config_file: Optional[Path] = None):
        self.config_file = config_file or (CONFIG_DIR / "blind-schedule.json")
...
def register(ctx, device_id, name, enabled, schedule):
    """註冊裝置"""
    core = BlindScheduleCore(ctx.obj.get("config"))
    result = core.register(device_id, name, enabled=enabled, schedule=schedule)
    console.print(Panel(f"[green]✓[/green] {result['message']}", title="窗簾排程器"))
