...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/power-watch/config.json` |
...
pip install paho-mqtt click rich
...
dependencies = ["paho-mqtt", "click", "rich"]

[project.scripts]
power-watch = "power_watch.cli:main"
```
...
```bash
power-watch [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
power-watch run --output json

# 詳細模式
power-watch run --verbose

# 靜音（適合 pipe 到其他工具）
power-watch run --quiet | jq .

# 查看設定
power-watch config show

# 匯出 JSON
power-watch export --output json > result.json
```
...
from .core import PowerWatchCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "power-watch" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "power-watch" / "data.db"
...
class PowerWatchCore:
    """
    核心邏輯類別。
    問題：用電量監測
    解法：透過 paho-mqtt, click 實現自動化處理。
    """
...
```bash
# Agent 呼叫範例
result=$(power-watch run --quiet --output json)
echo $result | jq .data
```
...
def call_power_watch(action="run") -> dict:
    proc = subprocess.run(
        ["power-watch", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
power-watch run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=power_watch --cov-report=term-missing

# 快速冒煙測試
power-watch run --help
power-watch config show
```
...
```python
import pytest
from power_watch.core import PowerWatchCore
...
*工具編號 #427 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
