...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/energy-tip/config.json` |
...
[project.scripts]
energy-tip = "energy_tip.cli:main"
```
...
```bash
energy-tip [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
energy-tip run --output json

# 詳細模式
energy-tip run --verbose

# 靜音（適合 pipe 到其他工具）
energy-tip run --quiet | jq .

# 查看設定
energy-tip config show

# 匯出 JSON
energy-tip export --output json > result.json
```
...
from .core import EnergyTipCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "energy-tip" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "energy-tip" / "data.db"
...
```bash
# Agent 呼叫範例
result=$(energy-tip run --quiet --output json)
echo $result | jq .data
```
...
def call_energy_tip(action="run") -> dict:
    proc = subprocess.run(
        ["energy-tip", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
energy-tip run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=energy_tip --cov-report=term-missing
...
```python
import pytest
from energy_tip.core import EnergyTipCore
...
*工具編號 #440 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
