...
| 介面 | CLI（`click` 框架） |
| 設定檔 | `~/.config/scene-flip/config.json` |
...
[project.scripts]
scene-flip = "scene_flip.cli:main"
```
...
```bash
scene-flip [OPTIONS] COMMAND [ARGS]...
```
...
# 指定輸出格式
scene-flip run --output json

# 詳細模式
scene-flip run --verbose

# 靜音（適合 pipe 到其他工具）
scene-flip run --quiet | jq .

# 查看設定
scene-flip config show

# 匯出 JSON
scene-flip export --output json > result.json
```
...
from .core import SceneFlipCore
...
def config():
    """顯示或修改設定"""
    import json, pathlib
    cfg_path = pathlib.Path.home() / ".config" / "scene-flip" / "config.json"
    if cfg_path.exists():
...
DB_PATH = pathlib.Path.home() / ".config" / "scene-flip" / "data.db"
...
{
  "name": "scene-flip",
  "description": "場景切換器。解決問題：家庭場景一鍵切換",
  "input_schema": {
    "type": "object",
    "properties": {
      "action": {
...
```bash
# Agent 呼叫範例
result=$(scene-flip run --quiet --output json)
echo $result | jq .data
```
...
def call_scene_flip(action="run") -> dict:
    proc = subprocess.run(
        ["scene-flip", action, "--output", "json", "--quiet"],
...
```bash
# 管線串接範例
scene-flip run --quiet --output json \
  | jq -r '.data' \
  | other-tool process
```
...
# 執行測試
pytest tests/ -v --cov=scene_flip --cov-report=term-missing
...
```python
import pytest
from scene_flip.core import SceneFlipCore
...
*工具編號 #422 ／ 類別 V — 智慧家庭與IoT ／ 計畫版本 v1.0 ／ 2026-04-27*
